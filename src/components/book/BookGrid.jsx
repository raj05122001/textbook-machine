...
const BookGrid = ({
  ...
  return (
    <div className={`${className}`}>
      ...
      {filteredAndSortedBooks.length > 0 && showStats && (
        <div className="mt-12 pt-8 border-t border-gray-200">
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-900">{bookStats.totalChapters}</div>
              <div className="text-sm text-gray-600">Total Chapters</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-blue-600">
                {Math.round(bookStats.averageProgress)}%
              </div>
              <div className="text-sm text-gray-600">Avg Progress</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-green-600">{bookStats.published}</div>
              <div className="text-sm text-gray-600">Published</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-purple-600">
                {bookStats.estimatedReadingTime}h
              </div>
              <div className="text-sm text-gray-600">Est. Reading</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-orange-600">
                {Math.round(bookStats.totalWords / bookStats.total || 0).toLocaleString()}
              </div>
              <div className="text-sm text-gray-600">Avg Words/Book</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-indigo-600">
                {Math.round(bookStats.totalChapters / bookStats.total || 0)}
              </div>
              <div className="text-sm text-gray-600">Avg Chapters/Book</div>
            </div>
          </div>
          ...
          <div className="mt-6">
            <div className="flex justify-between text-sm text-gray-600 mb-2">
              ...
              <span>{Math.round((bookStats.completed / bookStats.total) * 100) || 0}%</span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div className="flex h-3 rounded-full overflow-hidden">
                <div 
                  className="bg-green-500"
                  style={{ width: `${(bookStats.completed / bookStats.total) * 100 || 0}%` }}
                />
                <div 
                  className="bg-blue-500"
                  style={{ width: `${(bookStats.inProgress / bookStats.total) * 100 || 0}%` }}
                />
                <div 
                  className="bg-yellow-500"
                  style={{ width: `${(bookStats.draft / bookStats.total) * 100 || 0}%` }}
                />
                <div 
                  className="bg-purple-500"
                  style={{ width: `${(bookStats.published / bookStats.total) * 100 || 0}%` }}
                />
              </div>
            </div>
            <div className="flex justify-between text-xs text-gray-500 mt-1">
              ...
              <span className="flex items-center">
                ...
                In Progress ({bookStats.inProgress})
              </span>
              <span className="flex items-center">
                <div className="w-2 h-2 bg-yellow-500 rounded-full mr-1"></div>
                Draft ({bookStats.draft})
              </span>
              <span className="flex items-center">
                <div className="w-2 h-2 bg-purple-500 rounded-full mr-1"></div>
                Published ({bookStats.published})
              </span>
            </div>
          </div>
        </div>
