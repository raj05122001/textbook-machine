...
import {
    Download, FileText, BookOpen, Languages, GraduationCap,
} from 'lucide-react';
...
export default function PrimaryKnowledgePage() {
    ...
    async function handleDownload(item) {
        ...
        try {
            ...
            const finalName = mUtf8?.[1] ? decodeURIComponent(mUtf8[1]) : (mBasic?.[1] || file_name);
    ...
    return (
        <div className="min-h-[100dvh] w-full bg-[radial-gradient(1200px_600px_at_100%_-10%,rgba(124,58,237,0.08),transparent),radial-gradient(1000px_500px_at_-10%_110%,rgba(37,99,235,0.08),transparent)] py-10">
            <div className="mx-auto max-w-6xl px-4">
                <div className="mb-4 flex items-center justify-between">
                    <h1 className="text-xl font-semibold text-gray-900">Primary Knowledge</h1>
                ...
                {loading ? (
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                        {Array.from({ length: 4 }).map((_, i) => (
                            <div key={i} className="h-48 animate-pulse rounded-2xl border border-gray-200 bg-white/60" />
                        ))}
                    </div>
                ) : (
                    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            return (
                                <div key={item.id} className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
                                    <div className="bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 p-5 text-white">
                                        <div className="flex flex-wrap items-center justify-between gap-3">
                                            <div className="min-w-0">
                                                ...
                                                <h2 className="mt-2 truncate text-base font-semibold">{fileName}</h2>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="grid grid-cols-1 gap-6 p-6 sm:grid-cols-2">
                                        ...
                                        <MetaRow icon={BookOpen} label="Subject" value={item.subject?.subject_name} />
                                        <MetaRow icon={GraduationCap} label="Standard" value={item.standard} />
                                        <MetaRow icon={Languages} label="Language" value={item.book_language} />
                                    </div>

                                    <div className="flex flex-wrap items-center justify-between gap-3 border-t bg-gray-50/60 px-6 py-4">
                                        <div className="text-sm text-gray-600">
                                            <span className="font-medium text-gray-800">{toNiceDate(item.created_at)}</span>
                ...
                <p className="mt-6 text-center text-xs text-gray-500">
                    Tip: “Copy S3 key” → use anywhere in your admin tools.
                </p>
            </div>
        </div>
    );
